...
const Reports = () => {
    ...
    const loadReportData = async () => {
      ...
      const mockData = {
        overview: {
          ...
          departmentStats: [
            { name: 'Engineering', employees: 45, completed: 189, hours: 1124, completion: 85 },
            { name: 'Marketing', employees: 28, completed: 98, hours: 567, completion: 72 },
            { name: 'Sales', employees: 32, completed: 87, hours: 423, completion: 68 },
            { name: 'Operations', employees: 25, completed: 67, hours: 398, completion: 90 },
            { name: 'HR', employees: 12, completed: 28, hours: 189, completion: 95 },
            { name: 'Finance', employees: 14, completed: 20, hours: 146, completion: 82 }
          ],
          monthlyProgress: [
        ...
        progress: {
          ...
          topPerformers: [
            { name: 'Sarah Johnson', department: 'Engineering', completed: 12, hours: 67 },
            { name: 'Mike Chen', department: 'Marketing', completed: 9, hours: 52 },
            { name: 'Emily Rodriguez', department: 'Operations', completed: 11, hours: 58 }
          ]
        },
        completion: {
          ...
          completionTrends: [
            { course: 'Data Security Training', assigned: 156, completed: 148, rate: 94.9 },
            { course: 'Compliance & Ethics', assigned: 156, completed: 142, rate: 91.0 },
            { course: 'Safety Protocols', assigned: 134, completed: 118, rate: 88.1 },
            { course: 'Leadership Skills', assigned: 89, completed: 67, rate: 75.3 },
            { course: 'Project Management', assigned: 67, completed: 45, rate: 67.2 }
          ]
        }
      }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="card text-center">
          ...
          <div className="text-2xl font-bold text-text-dark">{reportData.totalEmployees}</div>
          <div className="text-sm text-text-medium">Total Employees</div>
        </div>
        <div className="card text-center">
          <Target className="h-8 w-8 text-green-600 mx-auto mb-2" />
          <div className="text-2xl font-bold text-text-dark">{reportData.coursesCompleted}</div>
          <div className="text-sm text-text-medium">Courses Completed</div>
        </div>
        <div className="card text-center">
          <Clock className="h-8 w-8 text-purple-600 mx-auto mb-2" />
          <div className="text-2xl font-bold text-text-dark">{reportData.totalHours.toLocaleString()}</div>
          <div className="text-sm text-text-medium">Total Hours</div>
        </div>
        <div className="card text-center">
          <Award className="h-8 w-8 text-yellow-600 mx-auto mb-2" />
          <div className="text-2xl font-bold text-text-dark">{reportData.certificates}</div>
          <div className="text-sm text-text-medium">Certificates</div>
        </div>
        <div className="card text-center">
          <TrendingUp className="h-8 w-8 text-primary mx-auto mb-2" />
          <div className="text-2xl font-bold text-text-dark">{reportData.averageCompletion}%</div>
          <div className="text-sm text-text-medium">Avg. Completion</div>
        </div>
        <div className="card text-center">
          <BookOpen className="h-8 w-8 text-indigo-600 mx-auto mb-2" />
          <div className="text-2xl font-bold text-text-dark">{reportData.activeEmployees}</div>
      ...
      <div className="card">
        <h3 className="text-lg font-semibold text-text-dark mb-4">Department Performance</h3>
            ...
            <tbody>
              {reportData.departmentStats.map((dept, index) => (
                <tr key={index} className="border-b border-background-light">
                  <td className="py-3 font-medium text-text-dark">{dept.name}</td>
      ...
      <div className="card">
        <h3 className="text-lg font-semibold text-text-dark mb-4">Monthly Learning Progress</h3>
  ...
  const renderProgressReport = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="card text-center">
          <div className="text-2xl font-bold text-blue-600">{reportData.inProgress}</div>
        ...
        <div className="card text-center">
          <div className="text-2xl font-bold text-gray-600">{reportData.notStarted}</div>
      ...
      {/* Weekly Progress */}
      <div className="card">
        <h3 className="text-lg font-semibold text-text-dark mb-4">Weekly Progress Trends</h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-background-dark">
      ...
      {/* Top Performers */}
      <div className="card">
        <h3 className="text-lg font-semibold text-text-dark mb-4">Top Performers</h3>
        <div className="space-y-3">
          {reportData.topPerformers.map((performer, index) => (
            <div key={index} className="flex items-center justify-between p-3 bg-background-light rounded-lg">
              <div>
                <div className="font-medium text-text-dark">{performer.name}</div>
                <div className="text-sm text-text-medium">{performer.department}</div>
              </div>
              <div className="text-right">
                <div className="font-medium text-text-dark">{performer.completed} courses</div>
                <div className="text-sm text-text-medium">{performer.hours} hours</div>
              </div>
            </div>
          ))}
        </div>
  ...
  const renderCompletionReport = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="card text-center">
          <div className="text-2xl font-bold text-text-dark">{reportData.totalAssignments}</div>
        ...
        <div className="card text-center">
          <div className="text-2xl font-bold text-primary">{reportData.completionRate}%</div>
          <div className="text-sm text-text-medium">Completion Rate</div>
        </div>
        <div className="card text-center">
          <div className="text-2xl font-bold text-text-dark">{reportData.averageTimeToComplete}</div>
      ...
      <div className="card">
        <h3 className="text-lg font-semibold text-text-dark mb-4">Course Completion Rates</h3>
        <div className="space-y-4">
          {reportData.completionTrends.map((course, index) => (
            <div key={index} className="border border-background-dark rounded-lg p-4">
              <div className="flex justify-between items-center mb-2">
                <h4 className="font-medium text-text-dark">{course.course}</h4>
                <span className="text-sm font-medium text-primary">{course.rate}%</span>
              </div>
              <div className="flex justify-between text-sm text-text-medium mb-2">
                <span>{course.completed} of {course.assigned} completed</span>
  ...
  return (
    <div className="container py-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-text-dark mb-2">Learning Reports</h1>
