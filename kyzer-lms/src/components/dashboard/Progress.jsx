    ...
    <div className="container py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold text-text-dark">Weekly Activity</h2>
          ...
          {/* Monthly Progress */}
          <div className="card">
            <h2 className="text-xl font-semibold text-text-dark mb-6">Monthly Progress</h2>
                ...
                <tbody>
                  {progressData.monthlyProgress.map((month, index) => (
                    <tr key={index} className="border-b border-background-light">
                      <td className="py-3 font-medium text-text-dark">{month.month}</td>
                      <td className="py-3 text-text-medium">{month.completed}</td>
                      ...
                      <td className="py-3">
                        <div className="flex items-center">
                          ...
                          <span className="text-sm text-text-medium">{month.hours}h</span>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          {/* Skill Progress */}
          <div className="card">
            <h2 className="text-xl font-semibold text-text-dark mb-6">Skill Development</h2>
              ...
              {progressData.skillProgress.map((skill, index) => (
                <div key={index} className="border border-background-dark rounded-lg p-4">
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="font-medium text-text-dark">{skill.skill}</h3>
                    <div className="flex items-center space-x-2">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium text-white ${getSkillColor(skill.level)}`}>
                        {getSkillLabel(skill.level)}
                      </span>
                      <span className="text-sm font-medium text-text-dark">{skill.level}%</span>
                    </div>
                  </div>
        ...
        <div className="space-y-6">
          {/* Learning Streak */}
          <div className="card">
            <h3 className="text-lg font-semibold text-text-dark mb-4">Learning Streak</h3>
          ...
          {/* Learning Goals */}
          <div className="card">
            <h3 className="text-lg font-semibold text-text-dark mb-4">Learning Goals</h3>
            <div className="space-y-4">
              {progressData.goals.map((goal) => (
                <div key={goal.id} className="border border-background-dark rounded-lg p-3">
                  <h4 className="font-medium text-text-dark text-sm mb-2">{goal.title}</h4>
                  <div className="flex justify-between text-xs text-text-medium mb-1">
                    <span>{goal.current} of {goal.target} {goal.type}</span>
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              {progressData.recentActivity.map((activity, index) => (
                <div key={index} className="flex items-start space-x-3 p-2 hover:bg-background-light rounded-lg transition-colors">
                  <div className="mt-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-1">
                      <span className="text-xs text-text-light">{formatDate(activity.date)}</span>
                      <span className="text-xs font-medium text-primary">+{activity.points} pts</span>
                    </div>
                  </div>
                </div>
              ))}
