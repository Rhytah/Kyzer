...
export function useEnrollment() {
  ...
  const updateProgress = async (courseId, progress) => {
    try {
      ...
      updatedEnrollments[enrollmentIndex] = {
        ...
        ...(progress >= 100 && !updatedEnrollments[enrollmentIndex].completed_at && {
  ...
  const getStats = async (userId) => {
    try {
      ...
      const inProgressCourses = enrollments.filter(e => e.progress > 0 && !e.completed_at).length
      ...
      return {
        ...
        recentAchievements: completedCourses > 0 ? [
          {
  ...
  return {
    ...
    inProgressCoursesCount: enrollments.filter(e => e.progress > 0 && !e.completed_at).length,
    
    // Get courses by status
    getInProgressCourses: () => enrollments.filter(e => e.progress > 0 && !e.completed_at),
    getCompletedCourses: () => enrollments.filter(e => e.completed_at),
    getRecentlyAccessedCourses: () => enrollments
